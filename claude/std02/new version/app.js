// new version: original version의 기능(데이터/CRUD/필터/진행률/일정 상태)을 그대로 이어받고
// 화면 디자인(나무 널판지 제목, 금속 진행률 바)만 새로 입힌 버전이다.
// 원본과 데이터가 섞이지 않도록 저장 키는 'todos-app-v2'를 사용한다.
...
// 금속 막대의 길이와 함께, 완료/전체 건수 옆에 퍼센트도 같이 표시한다.
function updateProgress() {
  ...
  progressBarFill.classList.toggle('is-empty', percent === 0);
  progressCount.textContent = `완료 ${completedCount} / 전체 ${total}`;
